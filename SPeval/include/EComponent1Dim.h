...
#ifndef ESsistMe_EComponent1Dim_H
#define ESsistMe_EComponent1Dim_H
#include <string>
#include <iostream>
#include <vector>
...
class EComponent1Dim : public EComponentBase
{
public:
  EComponent1Dim (ESpectrum1Dim *aSpectrum, int aWIdent, const string& aLabel, bool Root=false);
protected:
  bool Setup(void);
};
...
bool Component1Dim::Setup(void)
{   if(!ComponentBase::Setup()) return false;
    ...
    if(TheSpectrum)
        GetStore(0,TheSpectrum->GetNoOfEPoints()-1);
    return true;
}// Component1Dim::Setup
