...
void MnUserTransformation::add(MinuitParameter &MP, char * parName) {
    assert(std::find_if(theParameters.begin(), theParameters.end(), MnParStr(parName)) == theParameters.end());
  	if(!MP.isConst() && !MP.isFixed())
	{    theExtOfInt.push_back(theParameters.size());	}
    theCache.push_back(MP.value());
    theParameters.push_back(MinuitParameter(theParameters.size(), MP, parName));
}
#endif
