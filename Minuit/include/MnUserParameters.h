...
#if MINUIT_EXTENDED
  /// add Minuit parameter (as is)
  void add(MinuitParameter &MP, char * setName);
  /// add set of Minuit parameters
  void add(MinuitParameterSet &MPS);
  /// add Minuit parameter (as is)
  bool extract(MinuitParameter &MP);
  /// add set of Minuit parameters
  bool extract(MinuitParameterSet &MPS);
#endif
